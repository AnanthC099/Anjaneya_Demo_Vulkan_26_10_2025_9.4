...
static void EndOneShotCB(VkCommandBuffer cb)
{
    VkSubmitInfo si;
                                ...
                                VkSampler* outSampler)
{
    ...
    void* map = NULL;
    vkMapMemory(gCtx_Switcher.vkDevice, stagingMem, 0, imageSize, 0, &map);
    memcpy(map, pixels, (size_t)imageSize);
    ...
    vkCreateImage(gCtx_Switcher.vkDevice, &ici, NULL, outImg);

    VkMemoryRequirements imr;
    memset(&imr, 0, sizeof(imr));
    vkGetImageMemoryRequirements(gCtx_Switcher.vkDevice, *outImg, &imr);
    ...
    imai.allocationSize = imr.size;
    {
        ...
        for (i = 0; i < gCtx_Switcher.vkPhysicalDeviceMemoryProperties.memoryTypeCount; ++i)
        {
            if ((imr.memoryTypeBits & (1u << i)) != 0 &&
                (gCtx_Switcher.vkPhysicalDeviceMemoryProperties.memoryTypes[i].propertyFlags &
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT) == VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT)
            {
                imai.memoryTypeIndex = i;
                break;
            }
        }
    }
    vkAllocateMemory(gCtx_Switcher.vkDevice, &imai, NULL, outMem);
    ...
    }
    {
        ...
        vkCmdCopyBufferToImage(cb, staging, *outImg, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
        EndOneShotCB(cb);
    }
    {
        VkCommandBuffer cb = BeginOneShotCB();
    ...
    vkCreateImageView(gCtx_Switcher.vkDevice, &vci, NULL, outView);
    ...
    vkCreateSampler(gCtx_Switcher.vkDevice, &sci, NULL, outSampler);
                              ...
                              VkSampler* outSampler)
{
    ...
    memset(pix, 0, sizeof(pix));
    {
        ...
        for (f = 0; f < 6; ++f)
        {
            pix[f] = stbi_load(faces[f], &w, &h, &ch, STBI_rgb_alpha);
            if (!pix[f])
            {
                int i;
                for (i = 0; i < f; ++i) stbi_image_free(pix[i]);
                return VK_ERROR_INITIALIZATION_FAILED;
            }
        }
    }
    VkDeviceSize faceSize = (VkDeviceSize)w * (VkDeviceSize)h * 4;
    ...
    {
        void* map = NULL;
        vkMapMemory(gCtx_Switcher.vkDevice, stagingMem, 0, total, 0, &map);
        {
            uint32_t f;
            for (f = 0; f < 6; ++f)
            {
                memcpy((uint8_t*)map + (VkDeviceSize)f * faceSize, pix[f], (size_t)faceSize);
            }
        }
        vkUnmapMemory(gCtx_Switcher.vkDevice, stagingMem);
    }
    {
        int f;
        for (f = 0; f < 6; ++f) stbi_image_free(pix[f]);
    }
    ...
    /* copy six faces */
    {
        ...
        memset(regions, 0, sizeof(regions));
        {
            ...
            for (f = 0; f < 6; ++f)
            {
                regions[f].bufferOffset = (VkDeviceSize)f * faceSize;
                ...
                regions[f].imageExtent.width  = (uint32_t)w;
                regions[f].imageExtent.height = (uint32_t)h;
                regions[f].imageExtent.depth  = 1;
            }
        }
        vkCmdCopyBufferToImage(cb, staging, *outImg, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 6, regions);
        EndOneShotCB(cb);
    }
    /* to SHADER_READ_ONLY */
    {
...
static void UpdateShowcaseSequence(void)
{
    ...
    case SEQUENCE_SCENE0_HOLD:
    {
        ...
        {
            DWORD fadeElapsed = elapsed - K_SEQUENCE_SCENE0_FADE_IN_DELAY_MS;
            float t = (K_SEQUENCE_SCENE0_FADE_WINDOW_MS > 0)
                          ? Clamp01(fadeElapsed / (float)K_SEQUENCE_SCENE0_FADE_WINDOW_MS)
                          : 1.0f;
            gCtx_Switcher.gFade = t;
        }
        else
        {
            gCtx_Switcher.gFade = 1.0f;
        ...
        UpdateBlendFade(gCtx_Switcher.gFade);
        DWORD totalHold = K_SEQUENCE_SCENE0_FADE_IN_DELAY_MS +
                          K_SEQUENCE_SCENE0_FADE_WINDOW_MS +
                          K_SEQUENCE_SCENE0_VISIBLE_HOLD_MS;
        if (elapsed >= totalHold)
        {
    ...
    case SEQUENCE_SCENE0_FADE_OUT:
    {
        float t = Clamp01((now - sSequenceStateStartMs) / (float)K_SEQUENCE_FADE_MS);
...
VkResult vkResult = VK_SUCCESS;
    ...
    else if (gActiveScene == ACTIVE_SCENE_SCENE2)
    {
...
VkResult createVertexBuffer(void)
{
    ...
    memset((void*)&gCtx_Switcher.vertexData_position, 0, sizeof(GlobalContext_Switcher::VertexData));
   ...
   //VERTEX COLOR BUFFER 
    //#4 memset the global strucure variable
    memset((void*)&gCtx_Switcher.vertexData_texcoord, 0, sizeof(GlobalContext_Switcher::VertexData));
...
{
    auto createUniformBufferForScene = [&](GlobalContext_Switcher::UniformData& uniformData) -> VkResult
    {
        memset((void*)&uniformData, 0, sizeof(GlobalContext_Switcher::UniformData));
        ...
        vkBufferCreateInfo.pNext = NULL;
        vkBufferCreateInfo.flags = 0;
        vkBufferCreateInfo.size = sizeof(GlobalContext_Switcher::MyUniformData);
        vkBufferCreateInfo.usage = VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT;

        VkResult innerResult = vkCreateBuffer(gCtx_Switcher.vkDevice,
                                              &vkBufferCreateInfo,
                                              NULL,
                                              &uniformData.vkBuffer);
        if(innerResult != VK_SUCCESS)
        {
            return innerResult;
        }
        ...
        innerResult = vkAllocateMemory(gCtx_Switcher.vkDevice, &vkMemoryAllocateInfo, NULL, &uniformData.vkDeviceMemory);
        if(innerResult != VK_SUCCESS)
        {
            return innerResult;
        }

        innerResult = vkBindBufferMemory(gCtx_Switcher.vkDevice, uniformData.vkBuffer, uniformData.vkDeviceMemory, 0);
        if(innerResult != VK_SUCCESS)
        {
            return innerResult;
        }
...
VkResult createShaders(void)
{
    ...
    /* ===== Scene 1 shader modules ===== */
    {
        ...
        if (fp == NULL) return VK_ERROR_INITIALIZATION_FAILED;
        fseek(fp, 0L, SEEK_END);
        size_t size = (size_t)ftell(fp);
        fseek(fp, 0L, SEEK_SET);
        char* shaderData = (char*)malloc(size);
        size_t rv = fread(shaderData, size, 1, fp);
        fclose(fp);
        if (rv != 1) { free(shaderData); return VK_ERROR_INITIALIZATION_FAILED; }
        VkShaderModuleCreateInfo ci;
        memset(&ci, 0, sizeof(ci));
        ci.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
        ci.codeSize = size;
        ci.pCode = (uint32_t*)shaderData;
        vkResult = vkCreateShaderModule(gCtx_Switcher.vkDevice, &ci, NULL, &gShaderModule_vertex_scene1);
        free(shaderData);
        if (vkResult != VK_SUCCESS) return vkResult;
    }
    {
        const char* szFileName = "Shader_Scene1.frag.spv";
        FILE* fp = fopen(szFileName, "rb");
        if (fp == NULL) return VK_ERROR_INITIALIZATION_FAILED;
        fseek(fp, 0L, SEEK_END);
        size_t size = (size_t)ftell(fp);
        ...
        ci.codeSize = size;
        ci.pCode = (uint32_t*)shaderData;
        vkResult = vkCreateShaderModule(gCtx_Switcher.vkDevice, &ci, NULL, &gShaderModule_fragment_scene1);
        free(shaderData);
        if (vkResult != VK_SUCCESS) return vkResult;
    }
    ...
    /* ===== Scene 2 shader modules ===== */
    {
        ...
        ci.codeSize = size;
        ci.pCode = (uint32_t*)shaderData;
        vkResult = vkCreateShaderModule(gCtx_Switcher.vkDevice, &ci, NULL, &gShaderModule_vertex_scene2);
        free(shaderData);
        if (vkResult != VK_SUCCESS) return vkResult;
    }
    {
        ...
        ci.codeSize = size;
        ci.pCode = (uint32_t*)shaderData;
        vkResult = vkCreateShaderModule(gCtx_Switcher.vkDevice, &ci, NULL, &gShaderModule_fragment_scene2);
        free(shaderData);
        if (vkResult != VK_SUCCESS) return vkResult;
    }
...
VkResult createDescriptorSet(void)
{
    ...
    vkDescriptorBufferInfo_scene0.range = sizeof(GlobalContext_Switcher::MyUniformData);
    ...
    vkDescriptorBufferInfo_scene1.range = sizeof(GlobalContext_Switcher::MyUniformData);
    ...
    vkDescriptorBufferInfo_scene2.range = sizeof(GlobalContext_Switcher::MyUniformData);
...
VkResult createPipeline(void)
{
    ...
    /* ===== Scene 1 pipeline (uses Scene 1 shaders) ===== */
    {
        ...
        VkPipelineCache sc1Cache = VK_NULL_HANDLE;
        vkCreatePipelineCache(gCtx_Switcher.vkDevice, &pc, NULL, &sc1Cache);

        VkPipelineShaderStageCreateInfo stages[2];
        memset(stages, 0, sizeof(stages));
        stages[0].sType  = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
        stages[0].stage  = VK_SHADER_STAGE_VERTEX_BIT;
        stages[0].module = gShaderModule_vertex_scene1;
        stages[0].pName  = "main";
        stages[1].sType  = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
        stages[1].stage  = VK_SHADER_STAGE_FRAGMENT_BIT;
        stages[1].module = gShaderModule_fragment_scene1;
        stages[1].pName  = "main";
        ...
        blend.srcColorBlendFactor = VK_BLEND_FACTOR_CONSTANT_ALPHA;
        blend.dstColorBlendFactor = VK_BLEND_FACTOR_ONE_MINUS_CONSTANT_ALPHA;
        blend.colorBlendOp        = VK_BLEND_OP_ADD;
        blend.srcAlphaBlendFactor = VK_BLEND_FACTOR_CONSTANT_ALPHA;
        blend.dstAlphaBlendFactor = VK_BLEND_FACTOR_ONE_MINUS_CONSTANT_ALPHA;
        ...
        VkGraphicsPipelineCreateInfo sc1Info = vkGraphicsPipelineCreateInfo;
        sc1Info.pStages          = stages;
        sc1Info.stageCount       = 2;
        sc1Info.pColorBlendState = &cbs;

        vkResult = vkCreateGraphicsPipelines(gCtx_Switcher.vkDevice, sc1Cache, 1, &sc1Info, NULL, &gCtx_Switcher.vkPipeline_scene1);
        vkDestroyPipelineCache(gCtx_Switcher.vkDevice, sc1Cache, NULL);
        if (vkResult != VK_SUCCESS) return vkResult;
    }
    ...
    /* ===== Scene 2 pipeline ===== */
    {
        ...
        memset(&pc, 0, sizeof(pc));
        pc.sType = VK_STRUCTURE_TYPE_PIPELINE_CACHE_CREATE_INFO;
        VkPipelineCache sc2Cache = VK_NULL_HANDLE;
        vkCreatePipelineCache(gCtx_Switcher.vkDevice, &pc, NULL, &sc2Cache);
        ...
        VkGraphicsPipelineCreateInfo sc2Info = vkGraphicsPipelineCreateInfo;
        sc2Info.pStages = stages;
        sc2Info.stageCount = 2;
        sc2Info.pColorBlendState = &cbs;

        vkResult = vkCreateGraphicsPipelines(gCtx_Switcher.vkDevice, sc2Cache, 1, &sc2Info, NULL, &gCtx_Switcher.vkPipeline_scene2);
        vkDestroyPipelineCache(gCtx_Switcher.vkDevice, sc2Cache, NULL);
        if (vkResult != VK_SUCCESS) return vkResult;
    }
...
VkResult buildCommandBuffers(void)
{
    ...
    for(uint32_t i = 0; i < gCtx_Switcher.swapchainImageCount; i++)
    {
        ...
        for(uint32_t sceneIndex = 0; sceneIndex < 3; ++sceneIndex)
        {
            ...
            vkResult = vkResetCommandBuffer(targetBuffer, 0);
            if(vkResult != VK_SUCCESS)
            {
                fprintf(gCtx_Switcher.gpFile, "buildCommandBuffers() --> vkResetCommandBuffer() failed for image %d scene %d error %d\n", i, sceneIndex, vkResult);
                return vkResult;
            }
            ...
            vkResult = vkBeginCommandBuffer(targetBuffer, &vkCommandBufferBeginInfo);
            if(vkResult != VK_SUCCESS)
            {
                fprintf(gCtx_Switcher.gpFile, "buildCommandBuffers() --> vkBeginCommandBuffer() failed for image %d scene %d error %d\n", i, sceneIndex, vkResult);
                return vkResult;
            }

            VkClearValue vkClearValue_array[2];
            memset((void*)vkClearValue_array, 0, (sizeof(VkClearValue) * _ARRAYSIZE(vkClearValue_array)));
            vkClearValue_array[0].color = gCtx_Switcher.vkClearColorValue;
            vkClearValue_array[1].depthStencil = gCtx_Switcher.vkClearDepthStencilValue;
            ...
            vkRenderPassBeginInfo.renderPass = gCtx_Switcher.vkRenderPass;
            vkRenderPassBeginInfo.renderArea.offset.x = 0;
            vkRenderPassBeginInfo.renderArea.offset.y = 0;
            vkRenderPassBeginInfo.renderArea.extent.width = gCtx_Switcher.vkExtent2D_swapchain.width;
            vkRenderPassBeginInfo.renderArea.extent.height = gCtx_Switcher.vkExtent2D_swapchain.height;
            vkRenderPassBeginInfo.clearValueCount = _ARRAYSIZE(vkClearValue_array);
            vkRenderPassBeginInfo.pClearValues = vkClearValue_array;
            vkRenderPassBeginInfo.framebuffer = gCtx_Switcher.vkFrameBuffer_array[i];

            vkCmdBeginRenderPass(targetBuffer,
                                 &vkRenderPassBeginInfo,
                                 VK_SUBPASS_CONTENTS_INLINE);

            vkCmdBindPipeline(targetBuffer,
                              VK_PIPELINE_BIND_POINT_GRAPHICS,
                              pipelines[sceneIndex]);

            vkCmdSetBlendConstants(targetBuffer, blendConstants);

            vkCmdBindDescriptorSets(targetBuffer,
                                    VK_PIPELINE_BIND_POINT_GRAPHICS,
                                    gCtx_Switcher.vkPipelineLayout,
                                    0,
                                    1,
                                    &descriptorSets[sceneIndex],
                                    0,
                                    NULL);
            ...
            vkCmdBindVertexBuffers(targetBuffer,
                                   0,
                                   1,
                                   &gCtx_Switcher.vertexData_position.vkBuffer,
            ...
            vkResult = vkEndCommandBuffer(targetBuffer);
            if(vkResult != VK_SUCCESS)
            {
                fprintf(gCtx_Switcher.gpFile, "buildCommandBuffers() --> vkEndCommandBuffer() failed for image %d scene %d error %d\n", i, sceneIndex, vkResult);
                return vkResult;
            }
        }
    }
